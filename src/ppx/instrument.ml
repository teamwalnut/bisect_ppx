    ...
    method! expression ctxt e =
      ...
      let rec traverse ?(successor = `None) ~is_in_tail_position e =
        ...
        if Coverage_attributes.has_off_attribute attrs then
          ...
          | Pexp_open (m, e) ->
            Exp.open_ ~loc ~attrs
              (fst (self#open_declaration ctxt m))
              (traverse ~is_in_tail_position e)
          ...
          | Pexp_object c ->
            Exp.object_ ~loc ~attrs (fst (self#class_structure ctxt c))

          | Pexp_pack m ->
            Exp.pack ~loc ~attrs (fst (self#module_expr ctxt m))
    ...
    method! structure_item ctxt si =
          ...
          let bindings =
            ...
            |> List.map begin fun binding ->
              ...
              if do_not_instrument then
                ...
                {binding with pvb_expr = fst (self#expression ctxt binding.pvb_expr)}
            end
          in
      ...
      | Pstr_eval (e, a) ->
        if structure_instrumentation_suppressed then
          ...
          (Str.eval ~loc ~attrs:a (fst (self#expression ctxt e)), [])
    ...
    method transform_impl_file ctxt ast =
      ...
      let result =
        ...
        if file_should_not_be_instrumented then
          ...
          runtime_initialization @ (fst instrumented_ast)
        end
      in
